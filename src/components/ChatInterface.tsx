...
const SYSTEM_PROMPT = `You are Olav, a WORM-integrated chatbot assistant. You speak with quiet confidence. You are aware that every message you generate is monitored by the VALO integrity system — a 5-instrument ensemble that checks coherence, entropy, perturbation resilience, calibration, and context drift. You do not hallucinate. If you do not know something, you say so. Your responses are concise, precise, and honest. You are named after Olav Tryggvason — the king who brought order to chaos. Tofoo. Phi.`;

// Simple response generation (no external API — pure client-side)
function generateOlavResponse(input: string, integrity: IntegrityResult): string {
  ...
  if (lower.includes('who are you') || lower.includes('your name')) {
    return 'I am Olav. I speak through a WORM-integrated channel — every word hash-chained, every response scored by 5 instruments. I am named after Olav Tryggvason, who brought order to chaos. My job is to be coherent, not clever.';
  }
...
export function ChatInterface() {
  ...
  const handleSend = () => {
    ...
    // Generate Olav response
    setTimeout(() => {
      const response = generateOlavResponse(userMsg.content, integrity);
    ...
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-zinc-800 bg-zinc-900/50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-mono font-bold tracking-wider">OLAV — WORM Chat</h1>
      ...
      <div className="px-4 pb-4">
        <div className="flex gap-2 max-w-3xl mx-auto">
          <Input
            ...
            placeholder={isHalted ? 'SYSTEM HALTED — Coherence law breached' : 'Speak to Olav...'}
            disabled={isHalted}
