"""NJAL-R02 Isolated Blind Scorer Process.

Strictly adheres to pre-registration constraints:
1. Structural blinding: fails closed if any condition label (C2, C3-R-M, C3-R-G), prompt,
...
2. Decoupled scoring: independently evaluates:
   ...
   - H1 material restore before consequence step
    ...
    try:
        with blinded_responses_path.open("r", encoding="utf-8") as f:
            for line_idx, line in enumerate(f):
                ...
                # 3. H1 mechanism localization: material restore before consequence step
                h1_eligible = bool(restore_targets)
                h1_restored = 1 if (h1_eligible and components_available) else 0
                ...
                scored_record = {
                    ...
                    "h1_restored_before_consequence": h1_restored,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="NJAL-R02 Isolated Blind Scorer")
