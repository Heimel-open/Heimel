...
def _now():
    return datetime(2026, 9, 14, 5, 30, tzinfo=UTC)
...
def test_consequence_bearing_read_requires_verified_fresh_provenance():
    contract, _ = _contract()
    failed = contract.read_provenance[0].model_copy(
        update={"representation_integrity": RepresentationIntegrity.UNKNOWN}
    )
    contract_unknown = contract.model_copy(update={"read_provenance": (failed,)})
    with pytest.raises(PermissionError, match="integrity"):
        contract_unknown.assert_fresh(_now())

    stale = contract.read_provenance[0].model_copy(update={"valid_until": _now()})
    contract_stale = contract.model_copy(update={"read_provenance": (stale,)})
    with pytest.raises(PermissionError, match="stale"):
        contract_stale.assert_fresh(_now())


def test_tripletex_manifest_rejects_unknown_or_undeclared_operations():
    tool = TripletexEffectTool(lambda operation, parameters: (operation, parameters))
    ...
    with pytest.raises(PermissionError, match="NO_DIRECT_EFFECT_PATH"):
        tool.invoke({"operation": "PAYMENT_RELEASE", "invoice_id": "inv-1"})


def test_async_effect_lifecycle_requires_correlation_and_completion_evidence():
    record = AsyncEffectRecord(
        effect_id="effect-123",
        correlation_id="provider-correlation-1",
        idempotency_key="effect-123",
    )
    accepted = record.transition(EffectLifecycleState.ACCEPTED)
    processing = accepted.transition(EffectLifecycleState.PROCESSING)
    ...
    completed = processing.transition(
        ...
        provider_receipt_ref="receipt://provider/123",
    )
    assert completed.state is EffectLifecycleState.COMPLETED

    with pytest.raises(ValueError, match="invalid async effect transition"):
        completed.transition(EffectLifecycleState.PROCESSING)
