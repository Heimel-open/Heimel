...
class FunctionTool:
    def __init__(self, name: str, function: Callable[..., Any], capabilities: list[str] | None = None) -> None:
        self.name = name
        self.capabilities = list(capabilities or [])
        self._function = function

    def invoke(self, arguments: dict[str, Any]) -> Any:
        raise PermissionError("NO_DIRECT_EFFECT_PATH: use ValoGateway governed enforcement")
...
class ToolRegistry:
    ...
    def register(self, tool: FunctionTool, *, capability: str | None = None, target: str | None = None, credential_ref: str | None = None) -> EffectorHandle:
        ...
        resolved_capability = capability or (tool.capabilities[0] if len(tool.capabilities) == 1 else tool.name)
    ...
    def assert_effect_binding(self, handle: EffectorHandle, *, action_type: str, target: str) -> None:
    ...
    def _validate_governed_effect(self, handle: EffectorHandle, arguments: dict[str, Any], **context: Any) -> None:
        registered = self._handles.get(handle.name)
        if registered != handle or handle.registry_id != self._registry_id:
            raise PermissionError("effector handle is not owned by this boundary")
        validate = getattr(self._tools[handle.name], "_validate_governed_effect", None)
        if callable(validate):
            validate(arguments, **context)

    def _invoke_from_boundary(self, handle: EffectorHandle, arguments: dict[str, Any], proof: BoundaryProof, *, action_type: str, target: str) -> Any:
        _validate_boundary_proof(proof)
        self.assert_effect_binding(handle, action_type=action_type, target=target)
        return self._tools[handle.name]._invoke_from_boundary(arguments, proof)


def _invoke_tool_from_boundary(tool: object, arguments: dict[str, Any], proof: BoundaryProof) -> Any:
    _validate_boundary_proof(proof)
