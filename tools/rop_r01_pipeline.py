"""ROP-R01 Automated Two-Process Pipeline Orchestrator.
...
2. Structural blinding: Scorer receives blinded responses with NO condition labels, prompts, or structural scaffolding.
3. Decoupled scoring: Scorer evaluates mathematical correctness against the answer key,
   and dependency preservation against the frozen dependency map.
4. Gated unblinding: Conditions are unblinded only during post-scoring analysis.
"""
...
def run_pipeline(
    ...
    print("Structural Blinding: Emits responses stripped of condition labels & prompts.")
    print("=" * 75)

    with tempfile.TemporaryDirectory(prefix="rop_r01_executor_sandbox_") as sandbox_dir:
        ...
        executor_script = repo_root / "tools" / "rop_r01_executor.py"
    ...
    print("Invariants: No condition labels. Evaluates correctness and dependency preservation.")
    print("=" * 75)

    scorer_script = repo_root / "tools" / "rop_r01_scorer.py"
    scorer_cmd = [
        sys.executable,
        str(scorer_script),
        "--blinded-responses",
    ...
    print("Invariants: Evaluates H1, H2, H3 and Kill Rules from frozen manifest.")
    print("=" * 75)

    analyzer_script = repo_root / "tools" / "rop_r01_analyzer.py"
    analyzer_cmd = [
        sys.executable,
        str(analyzer_script),
        "--manifest",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ROP-R01 Two-Process Pipeline Orchestrator")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/ROP-R01"))
