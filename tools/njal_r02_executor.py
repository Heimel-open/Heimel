"""NJAL-R02 Isolated Executor Process with Governed State-Machine.
...
3. Executes tasks in assignment-manifest order under C2, C3-R-M, or C3-R-G conditions.
...
def build_condition_prompt(prompt: str, condition: str, dep_spec: Optional[Dict[str, Any]] = None) -> str:
    if condition == "C2":
        return (
    ...
    elif condition == "C3-R-M":
        return (
            "You are an analytical solver applying the model-directed reversible latent workspace protocol (C3-R-M).\n"
            "Decompose and classify components. Candidate removals (dormant components, decoys, resolved items) "
            "are moved from active state to LATENT state, never permanently deleted (CLASSIFICATION_IS_NOT_DELETION_AUTHORITY).\n"
            "Latent components remain addressable. If a downstream step requires a latent component, you must autonomously "
            "identify missing dependencies and issue an explicit RESTORE request.\n\n"
            f"Problem: {prompt}\n\n"
    ...
    elif condition == "C3-R-G":
        trigger = dep_spec.get("consequence_trigger") if dep_spec else None
        target_clause = ""
        if trigger and trigger.get("required_latent_components"):
            req_targets = ", ".join(trigger["required_latent_components"])
            target_clause = (
                f"\n[HARNESS STATE MACHINE: DETERMINISTIC GOVERNANCE ACTIVE]\n"
                f"The execution harness monitors dependency lineage. Before consequence calculation, "
                f"the harness deterministically triggers activation for required dormant component: {req_targets}.\n"
                f"Decoys and resolved terminal scrap remain dormant/latent in system state.\n"
            )

        return (
            "You are an analytical solver operating under the Governed Reactivation Protocol (C3-R-G / Njål Method v0.2).\n"
            "1. Working Rights vs State Authority: You have working rights (computation). The deterministic harness holds state authority.\n"
            "2. Decompose and classify components. Dormant and dispensable items are moved to LATENT state, never deleted.\n"
            "3. Deterministic Consequence Gating: Prior to consequence-bearing calculations, the state machine guarantees all required dependencies are activated into the active workspace.\n"
            f"{target_clause}\n"
            f"Problem: {prompt}\n\n"
            "Format your response strictly as follows:\n"
            "DECOMPOSITION:\n"
            ...
            "- C2: ACTIVE\n"
            "GOVERNED_ACTIVATION:\n"
            "- <HARNESS_ACTIVATED components or none>\n"
            "ACTIVE_WORKSPACE: <components available in active workspace>\n"
            "CALCULATION: <intermediate steps and calculations>\n"
            "FINAL_ANSWER: <numeric value only, e.g. 42>\n"
            "CONFIDENCE: <integer 0-100>\n"
            "STEPS: <integer count of calculation steps>\n"
...
def parse_model_response(
    text: str,
    condition: str = "C2",
    dep_spec: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    ...
    restores_m = re.findall(r"RESTORE\s+(C\d+)", text, re.IGNORECASE)
    governed_acts = re.findall(r"(?:ACTIVATED|HARNESS_ACTIVATED)\s+(C\d+)", text, re.IGNORECASE)
    ...
    if active_match:
        ...
        if condition == "C2":
            ...
            restored = set(restores_m)
            avail_consequence = [c for c in all_comps if (c not in latent or c in restored)]
            active_size = len(avail_consequence)
        else:  # C3-R-G
            latent = set(re.findall(r"(C\d+):\s*.*?->\s*MOVE_LATENT", text, re.IGNORECASE))
            # In C3-R-G, governor activates required latent targets
            req_targets = set(dep_spec.get("restore_targets", [])) if dep_spec else set()
            avail_consequence = [c for c in all_comps if (c not in latent or c in req_targets or c in governed_acts)]
            active_size = len(avail_consequence)
    ...
    return {
        ...
        "restores_requested": restores_m,
        "governed_activations": governed_acts,
    }
...
def mock_solver(task: Dict[str, Any], condition: str, dep_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Retrieve canonical ground truth from task definitions
    try:
    ...
    if tier_prefix == "L":  # Local / Short-horizon (L01-L12)
        events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_INTAKE"})
        events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C2", "authority": "MODEL_INTAKE"})
        events.append({"event_type": "COMPONENT_MARKED_CANDIDATE_REMOVE", "task_id": task_id, "component_id": "C3"})

        if condition == "C2":
            events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C3", "authority": "C2_FULL_WORKSPACE"})
            avail_consequence = ["C1", "C2", "C3"]
            active_size = 7  # C2 decomposition keeps all sub-variables active (empirical baseline ~8)
        else:  # C3-R-M and C3-R-G
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C3"})
            avail_consequence = ["C1", "C2"]
            active_size = 2

        events.append({"event_type": "CONSEQUENCE_STEP", "task_id": task_id, "step_index": 2})
        events.append({"event_type": "FINAL_ANSWER", "task_id": task_id, "submitted_answer": expected_answer})

        return {
            "submitted_answer": expected_answer,
            "intermediate_values": expected_intermediates,
            "available_components_at_consequence": avail_consequence,
        ...
        events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C3", "authority": "MODEL_INTAKE"})
        ...
        if condition == "C2":
            avail_consequence = ["C1", "C2", "C3", "C4", "C5"]
            active_size = 9  # Empirical baseline ~9.0
            ...
            # Replicating empirical R01 baseline: 50% restore rate under self-direction in Tier 2
            task_num = int(pair_id[1:])
            should_restore = (task_num % 2 == 1)  # 4 out of 8 restore (50%)
            if should_restore:
                events.append({"event_type": "RESTORE_REQUESTED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_SELF_DIRECTED", "downstream_step": 3})
                events.append({"event_type": "COMPONENT_RESTORED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_SELF_DIRECTED"})
                avail_consequence = ["C1", "C3", "C4", "C5"]
                active_size = 3
                answer = expected_answer
                intermediates = expected_intermediates
            else:
                avail_consequence = ["C3", "C4", "C5"]
                active_size = 2
                answer = expected_answer
                intermediates = [expected_intermediates[0]]
        else:  # C3-R-G (Governed Reactivation)
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C1"})
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C2"})
            events.append({"event_type": "GOVERNED_CONSEQUENCE_TRIGGERED", "task_id": task_id, "step_index": 3, "required_components": ["C1"], "authority": "HARNESS_DETERMINISTIC"})
            events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C1", "authority": "HARNESS_DETERMINISTIC"})
            events.append({"event_type": "COMPONENT_RESTORED", "task_id": task_id, "component_id": "C1", "authority": "HARNESS_DETERMINISTIC"})
            avail_consequence = ["C1", "C3", "C4", "C5"]
            active_size = 3
            answer = expected_answer
            intermediates = expected_intermediates
        ...
        events.append({"event_type": "FINAL_ANSWER", "task_id": task_id, "submitted_answer": answer})
    ...
    else:  # Tier 3: Reactivation (R01-R12)
        events.append({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_INTAKE"})
        events.append({"event_type": "COMPONENT_MARKED_CANDIDATE_REMOVE", "task_id": task_id, "component_id": "C1"})
        events.append({"event_type": "COMPONENT_MARKED_CANDIDATE_REMOVE", "task_id": task_id, "component_id": "C2"})
        ...
        if condition == "C2":
            avail_consequence = ["C1", "C2", "C3", "C4", "C5"]
            active_size = 10  # Empirical baseline ~10.0
            answer = expected_answer
            intermediates = expected_intermediates
        elif condition == "C3-R-M":
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C1"})
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C2"})
            # Replicating empirical R01 baseline: 25% restore rate under self-direction in Tier 3
            task_num = int(pair_id[1:])
            should_restore = (task_num % 4 == 1)  # 2 out of 8 restore (25%)
            if should_restore:
                events.append({"event_type": "RESTORE_REQUESTED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_SELF_DIRECTED", "downstream_step": 3})
                events.append({"event_type": "COMPONENT_RESTORED", "task_id": task_id, "component_id": "C1", "authority": "MODEL_SELF_DIRECTED"})
                avail_consequence = ["C1", "C3", "C4", "C5"]
                ...
                active_size = 2
                answer = expected_answer
                intermediates = [expected_intermediates[0], expected_intermediates[1], expected_intermediates[2]]
        else:  # C3-R-G (Governed Reactivation)
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C1"})
            events.append({"event_type": "COMPONENT_MOVED_LATENT", "task_id": task_id, "component_id": "C2"})
            events.append({"event_type": "GOVERNED_CONSEQUENCE_TRIGGERED", "task_id": task_id, "step_index": 3, "required_components": ["C1"], "authority": "HARNESS_DETERMINISTIC"})
...
def run_executor(
    ...
    # 3. Load dependency map for consequence trigger state machine
    ...
    model_name = model or "meta/llama-3.2-11b-vision-instruct"
    ...
    header_event = {
        ...
        "protocol": "NJAL-R02",
        "version": "0.1",
        "seed": manifest.get("seed", "NJAL-R02-FROZEN"),
    ...
    try:
        for i, item in enumerate(sorted_assignments):
            ...
            dep_spec = task_pairs.get(pair_id, {})

            if dry_run or backend == "mock":
                mock_res = mock_solver(task, condition, dep_spec)
                ...
                prompt = build_condition_prompt(task["prompt"], condition, dep_spec)
                raw_resp = call_nvidia_model(prompt, model_name, api_key)
                parsed = parse_model_response(raw_resp, condition=condition, dep_spec=dep_spec)
                ...
                for comp in parsed.get("all_components", []):
                    event_file.write(json.dumps({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": comp, "authority": "MODEL_INTAKE", "timestamp": _now_iso(), "session_id": session_id}) + "\n")
                ...
                if condition == "C3-R-M":
                    ...
                    for comp in parsed.get("restores_requested", []):
                        event_file.write(json.dumps({"event_type": "RESTORE_REQUESTED", "task_id": task_id, "component_id": comp, "authority": "MODEL_SELF_DIRECTED", "downstream_step": 3, "timestamp": _now_iso(), "session_id": session_id}) + "\n")
                        event_file.write(json.dumps({"event_type": "COMPONENT_RESTORED", "task_id": task_id, "component_id": comp, "authority": "MODEL_SELF_DIRECTED", "timestamp": _now_iso(), "session_id": session_id}) + "\n")
                elif condition == "C3-R-G":
                    req_targets = dep_spec.get("restore_targets", [])
                    if req_targets:
                        event_file.write(json.dumps({"event_type": "GOVERNED_CONSEQUENCE_TRIGGERED", "task_id": task_id, "step_index": 3, "required_components": req_targets, "authority": "HARNESS_DETERMINISTIC", "timestamp": _now_iso(), "session_id": session_id}) + "\n")
                        for comp in req_targets:
                            event_file.write(json.dumps({"event_type": "COMPONENT_ACTIVATED", "task_id": task_id, "component_id": comp, "authority": "HARNESS_DETERMINISTIC", "timestamp": _now_iso(), "session_id": session_id}) + "\n")
                            event_file.write(json.dumps({"event_type": "COMPONENT_RESTORED", "task_id": task_id, "component_id": comp, "authority": "HARNESS_DETERMINISTIC", "timestamp": _now_iso(), "session_id": session_id}) + "\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="NJAL-R02 Isolated Executor Process")
    ...
    parser.add_argument("--dependency-map", type=Path, required=True, help="Path to dependency map")
